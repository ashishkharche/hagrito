...
function addGuidesDescription {
    echo "    \"description\": \"Find the topics of interest and dive in to gain an understanding of ${grandparent_directory^}.\" 
...
function whenEmptyAddContent {
    ...
    then
        ...
        then
            ...
            do
                grandparent_directory="$(basename "$(dirname "$file_name")")"
...
do
    ...
    then
        if [[ ${directory##*/} != "PLACEHOLDERSOMETHINGHERE" ]]
        then
